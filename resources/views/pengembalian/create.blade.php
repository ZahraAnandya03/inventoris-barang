    ...
    <div class="content">
        <div class="container">
            ...
            <form action="{{ route('pengembalian.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="pb_id" class="form-label">Peminjaman</label>
                    <select name="pb_id" id="pb_id" class="form-control">
                        <option value="">-- Pilih Peminjaman --</option>
                        @foreach($peminjaman as $item)
                            <option value="{{ $item->pb_id }}">{{ $item->pb_nama_siswa }} - {{ $item->pb_no_siswa }}</option>
                        @endforeach
                    </select>
                    @error('pb_id')
            ...
            <a href="{{ route('pengembalian.index') }}" class="btn btn-secondary mt-3">Kembali ke Daftar Pengembalian</a>
        </div>
    </div>
</body>
</html>
